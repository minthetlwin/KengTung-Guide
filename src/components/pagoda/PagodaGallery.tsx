...
import type { PagodaGalleryConfig } from '../../data/pagodas'

const SPANS = ['lg:col-span-2 h-96', 'h-96', 'h-96', 'lg:col-span-2 h-72', 'lg:col-span-2 h-72']

interface PagodaGalleryProps {
  gallery: NonNullable<PagodaDetailDictionary['gallery']>
  albums: PagodaGalleryConfig['albums']
}

export function PagodaGallery({ gallery: g, albums }: PagodaGalleryProps) {
  const [openAlbumIndex, setOpenAlbumIndex] = useState<number | null>(null)
  const [photoIndex, setPhotoIndex] = useState(0)

  const openAlbum = openAlbumIndex === null ? null : albums[openAlbumIndex]
    ...
    <section id="photo-gallery" className="mx-auto w-full max-w-[1440px] scroll-mt-32 px-gutter md:px-gutter-lg">
          ...
          return (
            ...
            >
              ...
              <div className="absolute inset-x-5 bottom-5 flex flex-col gap-1">
                ...
                <h4 className="line-clamp-3 font-serif text-lg font-bold text-white">{a.title}</h4>
      ...
      {openAlbum && openAlbumCopy && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 p-4" onClick={close}>
          ...
          />
          <h4 className="mt-4 max-w-2xl text-center font-serif text-lg font-bold text-white">{openAlbumCopy.title}</h4>
          {openAlbumCopy.date && (
